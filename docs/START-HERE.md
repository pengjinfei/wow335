# 新会话接手（更新：2026-09-16 深夜，**第五个副本英雄古达克首轮基线已跑完**）

> **先读 [LESSONS](testing/LESSONS.md)，再读 [古达克副本勘测](testing/bosses/heroic-gd/README.md)。**
> 上一轮（达克萨隆要塞收尾 + 两条共享层修复）的交接在 [HANDOVER-2026-09-16](testing/HANDOVER-2026-09-16.md)，仍然有效。
>
> ## 当前状态：英雄古达克（Gundrak，map 604）
>
> 装备档 normal5-v1，难度英雄，`BotCheats=""`。**core 一行未改**；mod-playerbots 新分支
> `codex/gd-sladran-aoe` @ `715eb7c8`（AoE 乘子 `d29b2aff` + 第二刀回退 `78ad6020` + 包裹优先级 `8ebd5cd4` + **共享层保护链 `715eb7c8`**，叠在 `codex/shared-heal-los-recovery` 的 `5ef5adcc` 上，**未推 fork**）；
> mod-raidtest `codex/dtk-scenarios` @ `556c118`（古达克五个场景 conf，**未推 origin**）。
> **二进制 = 2026-09-16 深夜增量编译**，与 `715eb7c8` 一致。
> worldserver 以 `scripts/restart_world.sh r57a` 启动，日志 `/tmp/wow335-worldserver-r57a.log`，收尾时 IDLE。
> `AiPlayerbot.LogInGroupOnly` 已改回 1。
>
> | boss | 场景 | 本轮结果 | 下一步 |
> |---|---|---|---|
> | 斯拉德兰 29304 | 改前 0/5 → 第一刀（删掉「DPS 全程禁 AoE」）后 **2/10**。**第二刀（删掉「有包裹时禁 `dps assist`」）已试、判为净负面、已回退**：占比涨是分母塌了（包裹被打死 3.9→0.8/场，场次时长 −19%，击杀 0/5） | 累计 **4/22**。第三刀（按优先级选包裹，`8ebd5cd4`）**未证实有效但保留**。**包裹这条线到此为止**。**承伤侧已量完**：小怪承伤 45.6k（击杀场）→ 105.0k（团灭场）就是全部差别，boss 本体只 +7%；首死 13 次里非坦克 10 次**全是施法者**；不是没蓝（牧师地板 32–41%，两组无差）。**保命链整条断着**：保护之手 0/17（三级全学会）、牺牲之手 0、圣佑术 1、圣盾术 1、牧师给队友的痛苦压制 0、法师镜像 0、萨满治疗链 0。根因 = `PartyMemberToHeal.cpp:200` 那行 `return nullptr;`。**解 Spell.dbc 确认小怪伤害压倒性是白字物理 → 保护之手在这里有头寸（与因格瓦尔相反）**。→ **第四刀（打通保护链，`715eb7c8`）已做**：查出**两个叠在一起的缺陷**（死代码 `return nullptr;` + TBC 旧法术名 `"blessing of protection"`，WotLK 里叫 `"Hand of Protection"`），两个都修才有效；**共享层回归 9/9 全部击杀零死亡**；性能顾虑被反向指标否掉（tick diff 反而更好）。但**斯拉德兰上未测出效果**（击杀 2/7 → 0/7，保护之手只有 0.29 次/场——5 分钟冷却 + 「非坦克 ≤30% 血」门槛卡太晚）。**第五刀（DPS 目标固定为 boss，`129fb5ea`）已做**：执行率 51%、盗贼 tick 级 boss 目标 32%→76%，但结果指标全不动（击杀 0/9）——**头寸被第一刀吃掉了**（AoE 打开后法师的输出主体是地面 AoE，不看目标）。**第六刀（法师镜像触发器名 `6d2d5e2f`）已做**：`"high threat"` 这个触发器名全代码库没有 creator 注册 → 镜像永远不亮（**第三个「注册了但永远不亮」的缺陷**）。改后 0.20 → 1.00 次/场，**法师占小怪伤害 37% → 23%**、坦克 +63%、boss 残血 29.4% → 18.6%、时长 +13%，但击杀仍 0/5；共享层回归 8/9（唯一失败场次里镜像 0/6 触发，惰性、与改动无关）。**第七刀（防骑奉献相关性 27→52，`39ac1a7f`）已做，是第二条有效改动**：奉献饱和度 37%→54%、末段断档 36.5s→11.8s、打到 boss 的伤害 +17%、承伤 −14%、**团灭场 boss 残血 18.6% → 5.0%**（三场剩 8%/6%/11%），**击杀 0/5 → 2/5**；**共享层回归 9/9 全部零死亡**（上一轮那场凯利丝塔萨团灭没复现，确认是噪声）。**第八刀（神圣之锤高相关性节点）已试、净负面、已回退**（`87282855`）：靶子指标 +136% 但击杀 2/5 → 0/5、奉献被挤掉 11%、坦克接住的小怪伤害 −26%——**它和奉献抢同一个 GCD**。**神圣愤怒也已否掉**（只对亡灵/恶魔有效，而斯拉德兰是人型、小怪是野兽，零头寸）。累计 **6/57**。剩余候选：正义防御（2 场 0 次成功）、制裁之手（48%）；或换思路——治疗 1237 HPS 是硬上限、承伤 210.8k/场仍是主约束。原头寸：：8.3 个/场、几乎不重叠（同时 1.07 个）、**45% 在坦克身上、77% 根本没困住人**（自身对照：被包裹后到下次施法的间隔在平时正常范围内），只有牧师那 16% 是真被困（中位 2.7s、最长 16.8s），而 8 场有死亡的场次里 5 场首死落在牧师被困后 15 秒内（基础率 16%，富集 3.8 倍）。→ **不是打包裹没用，是打错了包裹**：`AttackSnakeWrapAction` 取列表里第一个、不排序。候选改法与对照指标见 boss 记录。另两项还没量：承伤侧、小怪仍只打死 40% |
...
> （载具犀牛没了、骑手还在）。乘客是召唤物没有 spawn guid，要清干净需要「按 entry 移除」的夹具能力——
> 与巨像那条待办同族，都是**新增功能**。
>
> **本轮又踩一次的坑**：`raidtest los` 的 `los` 是从 `z1+2` 量的，**必须逐点用实测地面高度当 z1**。
> 用统一 z 扫出来的「走廊对 boss 全部可见」是假的，正确 z 下**全部不可见**——两个场景差点写成开不了怪的坐标。
>

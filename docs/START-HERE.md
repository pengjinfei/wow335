...
1. **Ingvar 的真凶已定位：Woe Strike（59735）。** 因格瓦尔英雄 P2 每 15–20 秒给坦克挂
   ...
   **干净环境复测（run332）后已有统计支持**：合并 run329/331/332 的 11 场完成 attempt，
   无视线秒 = 0 的 6 场全部击杀，无视线 > 0 的 5 场里 4 场团灭；四场团灭全部有无视线时间，
   七场击杀里六场为 0，双尾 Fisher **p = 0.015**。
   同时**推翻**了先前那条「Woe 反射伤害 ≥2 万即团灭」的阈值——run332/seq1 反射 48,768 却击杀、
   seq4 只有 16,515 却团灭，那是样本巧合，不得再引用。
   **仍不要直接动手**：只有 11 场；且散开逻辑一改会同时影响躲前锥（20%→67% 的来源），
   须再补一轮干净的 10–15 场，并把「前锥命中率」一并纳入回归。
   可选修法：散开落点加入「对被驱散者的视线」校验，或给驱散补一个类似
   `reach party member to heal` 的「移动到能看见」动作。

   **环境三处问题（均已处理，见台账）**：bot 自动装备升级件会静默破坏固定装备档
   （`AiPlayerbot.AutoEquipUpgradeLoot` 已由 1 改为 **0**）；夹具校验把「还没复活」误报成
   「装备不合格」（已修，mod-raidtest `dfc7372`）；长时间连续运行后编排会不稳
   （首场 300 秒超时、准备点门禁失败），**根因未定位**，每轮前需清
   `account_instance_times` 与 `instance WHERE map=574` 并重启 worldserver。

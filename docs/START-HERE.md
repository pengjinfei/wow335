# 新会话接手（更新：2026-09-15 夜，因格瓦尔：三项落地，30 场 73%，待决定是否收尾）
...
> 两项已修：视线恢复（`4eade4b7`，UK 层，团灭场无视线秒 9/12/17 → 全 0，击杀率未动）、
> **治疗自保（`20e3e2b0`，共享层）**：窗口内自治 12%→34%，但**击杀 14/19 = 74%（p=0.675）、
> 治疗阵亡 7/19（p=0.128），两项都不显著**；最初按 10 场写的 9/10 / p=0.07 已被补测（run539 1/5）推翻。
> 共享层回归 9/9 零死亡。
> **萨满补位治疗已做（`1e285dff`，共享层）但在本 boss 是零效应**：11 场触发 1 次、执行 0 次，
> 完整判据窗口只有 3.3 秒/场（此前 9.9 秒是漏算了补位者自身存活与法力）。
> 同轮用新的 `casting=` 探针证伪了"治疗被长读条锁住"（存活期只有 10% 在读条）。
> **当前率 22/30 = 73%（CI 56–86%），待决定**：继续找第三个杠杆，还是按"隔离 boss 战 73%"收尾转下一个副本。
> 第二候选：治疗被自己的长读条锁住（`casting=` 只读探针 `44f5947` **已写未编译**，先量再改）。
> 两条已证伪勿重走：Roar 垫血（命中前血量中位 87%）、圣骑士保护之手（三个致死源全是暗影）。
> 详见 [因格瓦尔记录](testing/bosses/heroic-uk-ingvar/README.md) 2026-09-15 两节。

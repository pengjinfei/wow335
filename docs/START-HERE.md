...
| modules/mod-raidtest | dev | `325c226`（控制链开怪门禁、`raidtest los` 探针、开场等待、清图标、链式准备、`PrerequisiteMinBossDistanceBossEntry`；叠在 `f0dad96` 之上） |

> 构建树二进制 = 工作区最新提交（mod-raidtest `325c226`、mod-playerbots `611211b5`），无未编译改动。
...
### 凯利丝塔萨首次击杀——隔离形态 run419 2/5（2026-09-12 上午）
...
**接手第一件事**：该场景已加 `AttemptStartDelaySeconds = 300`，重启后复跑 5 场拿干净样本（约 40 分钟）；之后看寒气跳跃阈值
（`IntenseColdTrigger stackThreshold = 5`，真人保持 1–2 层）。

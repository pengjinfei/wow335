...
> `codex/gd-sladran-aoe` @ `78ad6020`（斯拉德兰 AoE 乘子 `d29b2aff` + 第二刀回退 `78ad6020`，叠在 `codex/shared-heal-los-recovery` 的 `5ef5adcc` 上，**未推 fork**）；
> mod-raidtest `codex/dtk-scenarios` @ `556c118`（古达克五个场景 conf，**未推 origin**）。
> **二进制 = 2026-09-16 22:19 增量编译**，与 `78ad6020` 一致（= 第一刀生效、第二刀已回退）。
> worldserver 以 `scripts/restart_world.sh r54a` 启动，日志 `/tmp/wow335-worldserver-r54a.log`，收尾时 IDLE。
> `AiPlayerbot.LogInGroupOnly` 已改回 1。
>
> | boss | 场景 | 本轮结果 | 下一步 |
> |---|---|---|---|
> | 斯拉德兰 29304 | 改前 0/5 → 第一刀（删掉「DPS 全程禁 AoE」）后 **2/10**。**第二刀（删掉「有包裹时禁 `dps assist`」）已试、判为净负面、已回退**：占比涨是分母塌了（包裹被打死 3.9→0.8/场，场次时长 −19%，击杀 0/5） | 缺口约 100k。**包裹头寸已量完**：8.3 个/场、几乎不重叠（同时 1.07 个）、**45% 在坦克身上、77% 根本没困住人**（自身对照：被包裹后到下次施法的间隔在平时正常范围内），只有牧师那 16% 是真被困（中位 2.7s、最长 16.8s），而 8 场有死亡的场次里 5 场首死落在牧师被困后 15 秒内（基础率 16%，富集 3.8 倍）。→ **不是打包裹没用，是打错了包裹**：`AttackSnakeWrapAction` 取列表里第一个、不排序。候选改法与对照指标见 boss 记录。另两项还没量：承伤侧、小怪仍只打死 40% |

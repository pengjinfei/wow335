# 新会话接手（更新：2026-09-18 晚，**莫拉比前置拉怪缺陷已修，run664 = 5/5 零死亡，待独立复现**）
...
>
> ## 当前状态：英雄古达克（Gundrak，map 604）——**迦尔达拉稳定；莫拉比前置已修、5/5 待复现；斯拉德兰仍未稳定**
>
...
> | # | boss | 场景 | 结果 | CI | 判定 |
> |---|---|---|---|---|---|
> | 1 | 斯拉德兰 29304 | `heroic-gd-sladran-disc-n5` | 合并后 **3/5**；平台阵位 **0/5**（已回退） | 0–43%（平台组） | 不稳定；别重试平台阵位 |
> | 2 | 莫拉比 29305 | `heroic-gd-moorabi-n5` | 修复后 **run664 = 5/5 零死亡**（160.7–242.5 秒）；run662/663 另 3 场击杀 | 56.6–100%（run664） | 前置拉怪缺陷已定位并修复；**待独立复现一轮** |
> | 3 | 德拉克瑞巨像 29307 | `heroic-gd-colossus-n5` | **5/5 零死亡**，64.374–68.137 秒（run651–652） | 57–100% | **隔离 boss 战基线通过**；不是连续副本通关 |
...
>
> **2026-09-18 晚本轮修的是框架自己的缺陷（mod-raidtest `AttemptRunner`，已编译、未提交）**：
> 普通前置接近原本只让坦克探路、其余人留在准备点并压制其 `attack tagged`，把清怪变成坦克单挑。
> run661 日志坐实：三次 `prerequisite approach` 目标都是上层平台的 127062 Fire Weaver，
> 坦克被单放到下层走廊全部 `los=false`，4 个跟随者全程 `combat=false target=none` 钉在准备点，
> `BeginPullForAll` 只有 leader 成功却被记为拉怪成功；81.7 秒坦克单人在平台被 Lancer 打死，
> 200 秒清怪超时。修复后 run664 完整链路 **5/5 零死亡**、前置清怪 53.2–74.6 秒。
> **boss 阶段的 `CanNotReachTarget`（run640 seq4）本轮未复现、也仍未归因，与本修复是两件事。**
>
>
...
>
> **接手第一件事：四选一**（按代价排序）——
> (a) **莫拉比独立复现一轮 5 场**（`raidtest run heroic-gd-moorabi-n5 --attempts 5`，先清 `account_instance_times`；不需编译）；
> (b) **凶残的艾克**勘测建场景（这一轮的遗漏）；
> (c) **推 fork** 把 12 个未推送提交备份掉；

# 新会话接手（更新：2026-09-16 凌晨，第四个副本达克萨隆要塞：四个 boss 三通一卡，靶子 = King Dred 的恐惧）

> **当前在做**：英雄达克萨隆要塞（Drak'Tharon Keep，map 600）。用户 2026-09-15 夜让「按项目进展
> 自行选下一个副本」；因格瓦尔按 30 场 73% 收尾不再攻坚。
>
> | boss | 场景 | 基线 | 判定 |
> |---|---|---|---|
...
> | **恐怖之王德雷德 King Dred** | `heroic-dtk-dred-n5` | **15/16 = 94%**（CI 72–99%，修完共享层视线缺陷后） | **正常规则通关** |
>
> 三个通关的 boss **bot 策略一行未改**；唯一的源码改动是 mod-raidtest 的一道开怪门禁（诺沃斯必需）。
>
> **德雷德已通关，靶子清空。** 路径是：恐惧被证伪（击杀场同样吃满恐惧）→ 目标切换也不是问题
> → 真因是**治疗被视线过滤饿死**（LESSONS 第 0 条那个一直没修的共享层缺陷）。
> **已修**（mod-playerbots `9134211d`，分支 `codex/shared-heal-los-recovery`）：
> 新增 `party member to heal no los` 取值只给「走过去」这条链用，施法仍用带视线的取值。
> 中间量 无视线占比 **22% → 3%**、最长块 27 秒 → 6 秒、治疗频率 0.37 → 0.48 次/秒；
> 击杀 3/5 → **15/16（94%）**，Fisher p=0.128 不显著（改前只有 5 场）。
> **回归三个旧 boss 9/9**。
>
> **同族第二条也修了**（`5ef5adcc`）：`main tank` 取值不再按视线过滤。改前非坦克位
> **20%** 的战斗秒取不到主坦（法师 30% / 萨满 27%），那些秒里盗贼嫁祸、法师隐形、牧师渐隐、
> 给坦克的 buff、以及"追敌不离主坦"护栏**整组静默跳过**；改后 **0%**。德雷德 7/7，回归再 9/9。
> 量它的只读探针 `mt=` 已常驻 mod-raidtest 采样（`9d36278`）。
> 施法型取值（驱散/复活/团队 buff）**故意不动**——隔墙放不出去，按视线过滤是对的。
>
> **达克萨隆要塞四个 boss 至此全部通关**（隔离 boss 战口径）。下一个副本待用户指定；
> 候选按等级档是紫罗兰监狱(608) / 古达克(604) / 石之大厅(599)。
>
> **本轮新增的三条通用事实（换场景必读）**：
> 1. `raidtest_accounts` 按 scenario_key 存槽位映射，**新场景键没有映射会去新建角色并因重名失败**
>    （`failed to create character for slot 0`）。照抄一份既有映射行，SQL 见 Trollgore 记录。
> 2. **只有走过「前置清怪 → 恢复」这条路，框架才会在恢复结束时把队伍传送到开怪点**；
>    没有前置怪的场景队伍停在准备点开怪。所以「准备点必须远离 boss」的场景**至少要留一只前置怪**。
> 3. **boss 会巡逻时，开怪点必须对整条巡逻线都有视线**，不能只对出生点（德雷德栽了一次）。
>
> **上一个副本（乌特加德城堡·因格瓦尔）收尾状态**：22/30 = 73%（CI 56–86%），三项落地
> （`4eade4b7` 视线恢复、`20e3e2b0` 治疗自保、`1e285dff` 萨满补位治疗，后者在该 boss 零效应）。
> 详见 [因格瓦尔记录](testing/bosses/heroic-uk-ingvar/README.md) 与 [交接文档](testing/HANDOVER-2026-09-14.md)。
>
> **运行状态**：worldserver 日志 `/tmp/wow335-worldserver-r40a.log`，二进制含 2026-09-16 那次增量编译
> （mod-raidtest `CombatTrigger.cpp`，**未提交**）。开跑前记得清 `account_instance_times`；
> 重启后第一条 FIFO 命令会被吞、要重发并回读。

...
- 斯卡瓦尔德与达隆：run127 及 2026-09-09 回归 run279/280 共三次零死亡击杀（48.6/51.5/49.0 秒）。**但该场景的前置清怪链自 `c460d05` 加入起从未启动过**：准备点 `(75,0,117)` 对整片小怪区无视线（最近目标 19.26 码时 `los` 仍为 false），对 boss 也无可用视线。需要实测集结点（可复用 `heroic-uk-ingvar` 的 `PrerequisiteX/Y/Z`），或让编排层在超出视线时先带队接近而不是第一个 tick 直接 abort。详见 [UK 机制审计](testing/bosses/heroic-uk/MECHANICS-AUDIT.md)。

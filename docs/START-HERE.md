# 新会话接手（更新：2026-09-17，**第五个副本英雄古达克一轮已收尾，未打穿**）

> **先读 [LESSONS](testing/LESSONS.md)，再读本轮交接 [HANDOVER-2026-09-17](testing/HANDOVER-2026-09-17.md)。**
> 副本级勘测在 [古达克副本勘测](testing/bosses/heroic-gd/README.md)。
>
> ## 当前状态：英雄古达克（Gundrak，map 604）——**5 个 boss 只打通 1 个**
>
> 装备档 normal5-v1（ilvl 上限 187），难度英雄，`BotCheats=""`，
> 口径均为**隔离 boss 战**（不是连续通关一遍副本）。
>
> | # | boss | 场景 | 结果 | CI | 判定 |
> |---|---|---|---|---|---|
> | 1 | 斯拉德兰 29304 | `heroic-gd-sladran-disc-n5` | **3/19** | 6–38% | 打不过（主要卡点） |
...
>
> **仓库状态**：core **一行未改**（`codex/an-formation-despawn-crash` @ `fa702e6a5`）；
> mod-playerbots `codex/gd-sladran-aoe` @ `bcd130eb`，**6 个提交未推 fork**；
> mod-raidtest `codex/dtk-scenarios` @ `556c118`，已同步；wow335 管理库 main **6 个 docs 提交未推**。
> **二进制 = 第十刀回退后重新编译的版本**，功能 = 第七刀态 + 共享层 `CombatOpeningTrigger`（未在任何策略注册）。
...
> 要继续只能动治疗侧（实测治疗**没有**低血优先：0–20% HP 覆盖率 29%，和 60–80% 一样）。
>
> **接手第一件事：四选一**（按代价排序）——
> (a) **莫拉比**加只读探针查「谁把 boss 拉进战斗」（不用编译，一次重启，最便宜）；
> (b) **凶残的艾克**勘测建场景（这一轮的遗漏）；
> (c) **推 fork** 把 12 个未推送提交备份掉；
> (d) **斯拉德兰治疗侧**（最贵，要编译，且瓶颈是硬上限）。
> 德拉克瑞巨像需要 mod-raidtest 新增「按 entry 声明/移除召唤物」能力，属**新增功能**，动手前确认口径。
>

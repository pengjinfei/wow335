...
> ⚠️ 工作区里有**未编译**的链式准备改动（见「链式准备」一节）；构建树二进制 = run408 那版（含剑刃乱舞修复）。
...
### 链式准备（2026-09-11 深夜，已改代码与配置、**未编译未验证**）
...
下一步：征得同意后增量编译 → 先跑 `heroic-nexus-telestra-n5` 5 场（守卫 + boss 完整链路）→ 再跑链式看走到哪一步。

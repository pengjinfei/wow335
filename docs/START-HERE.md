...
> `codex/gd-sladran-aoe` @ `8ebd5cd4`（AoE 乘子 `d29b2aff` + 第二刀回退 `78ad6020` + 包裹优先级 `8ebd5cd4`，叠在 `codex/shared-heal-los-recovery` 的 `5ef5adcc` 上，**未推 fork**）；
> mod-raidtest `codex/dtk-scenarios` @ `556c118`（古达克五个场景 conf，**未推 origin**）。
> **二进制 = 2026-09-16 22:4x 增量编译**，与 `8ebd5cd4` 一致。
> worldserver 以 `scripts/restart_world.sh r55a` 启动，日志 `/tmp/wow335-worldserver-r55a.log`，收尾时 IDLE。
> `AiPlayerbot.LogInGroupOnly` 已改回 1。
>
> | boss | 场景 | 本轮结果 | 下一步 |
> |---|---|---|---|
> | 斯拉德兰 29304 | 改前 0/5 → 第一刀（删掉「DPS 全程禁 AoE」）后 **2/10**。**第二刀（删掉「有包裹时禁 `dps assist`」）已试、判为净负面、已回退**：占比涨是分母塌了（包裹被打死 3.9→0.8/场，场次时长 −19%，击杀 0/5） | 累计 **4/22**。第三刀（按优先级选包裹，`8ebd5cd4`）**未证实有效但保留**。**包裹这条线到此为止**。**承伤侧已量完**：小怪承伤 45.6k（击杀场）→ 105.0k（团灭场）就是全部差别，boss 本体只 +7%；首死 13 次里非坦克 10 次**全是施法者**；不是没蓝（牧师地板 32–41%，两组无差）。**保命链整条断着**：保护之手 0/17（三级全学会）、牺牲之手 0、圣佑术 1、圣盾术 1、牧师给队友的痛苦压制 0、法师镜像 0、萨满治疗链 0。根因 = `PartyMemberToHeal.cpp:200` 那行 `return nullptr;`。**解 Spell.dbc 确认小怪伤害压倒性是白字物理 → 保护之手在这里有头寸（与因格瓦尔相反）**。→ **下一刀 = 打通 `PartyMemberToProtect`，共享层，必须回归三个旧 boss**。⚠ `GDActions.cpp` 里还有临时探针 `snakewrap_pick` 要删。原头寸：：8.3 个/场、几乎不重叠（同时 1.07 个）、**45% 在坦克身上、77% 根本没困住人**（自身对照：被包裹后到下次施法的间隔在平时正常范围内），只有牧师那 16% 是真被困（中位 2.7s、最长 16.8s），而 8 场有死亡的场次里 5 场首死落在牧师被困后 15 秒内（基础率 16%，富集 3.8 倍）。→ **不是打包裹没用，是打错了包裹**：`AttackSnakeWrapAction` 取列表里第一个、不排序。候选改法与对照指标见 boss 记录。另两项还没量：承伤侧、小怪仍只打死 40% |

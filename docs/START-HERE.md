# 新会话接手（更新：2026-09-18，**莫拉比北侧前置已复验，但 boss 战仍不稳定**）

> **先读 [LESSONS](testing/LESSONS.md)，再读本轮交接 [HANDOVER-2026-09-18](testing/HANDOVER-2026-09-18.md)。**
> 副本级勘测在 [古达克副本勘测](testing/bosses/heroic-gd/README.md)。
>
> ## 当前状态：英雄古达克（Gundrak，map 604）——**迦尔达拉稳定；莫拉比已定位到位移/寻路异常；斯拉德兰仍未稳定**
>
...
> | 2 | 莫拉比 29305 | `heroic-gd-moorabi-n5` | **8 击杀 / 1 aborted / 1 timeout**（10 个 boss 样本；run645 前置作废不计） | — | 北侧前置可复现；仍有 boss 异常，未通关 |
...
>
> **接手第一件事：四选一**（按代价排序）——
> (a) **莫拉比**若继续取证，先在启动前启用 `movement.chase` 日志，再临时记录 `TargetedMovementGenerator` 的置标分支；不要原样重跑；

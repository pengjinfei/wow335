...
| modules/mod-playerbots | **codex/nexus-trash-cc-pull** | `0c77db4b`（清怪控制链：指派/上控/不放 AoE/按序击杀，叠在 `eb91552d` 之上；**未推 fork**） |
| modules/mod-raidtest | dev | `72fb074`（控制链开怪门禁 `PrerequisiteCcWaitSeconds`、`raidtest los` 视线探针、`AttemptStartDelaySeconds`、每场清图标；叠在 `f0dad96` 之上） |

> ⚠️ **当前运行/构建树里的 worldserver 二进制比 `0c77db4b` 少一处改动**：`CrowdControlProtectionMultiplier` 里
> 「间接 AoE 名单先于 IsPositive 判断」（让剑刃乱舞也被压住）是 run407 之后改的，**没编译**。接手先增量编一次。
...
基线 12 场是 9/12；击杀顺序每场都是骷髅→十字→方块→月亮。代价是清怪 69–81 秒（基线 33 秒）。

...
> | 迦尔达拉 29306 | **2/5 但口径是错的**：三场团灭全部是我场景里留着的房内犀牛群（3 只载具 + 3 名骑手，被我数成 3 只）参战，两场击杀里它们没参战 | **先把犀牛群夹具移除重跑 5 场**拿干净基线；旧 5 场作废 |
> | 德拉克瑞巨像 29307 | **未建** | 框架阻塞 | 需要 mod-raidtest 新增「按 entry 指定必须清掉的召唤物」能力（**新增功能**，动手前确认口径） |
>
> **接手第一件事：三选一**——(a) 迦尔达拉夹具移除犀牛群后重跑基线（不需编译，最省，且死因已查清）；
> (b) 给莫拉比加探针（要编译）；(c) 量斯拉德兰那三个头寸（要 `LogInGroupOnly=0`，要编译才能加计数则视做法而定）。
>

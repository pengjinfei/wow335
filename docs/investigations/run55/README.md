...
> **最新验证已完成**：见 [run56–59 结果](VALIDATION.md)。两次真实击杀，run59零死亡；原run55长空窗未复现，主坦持续攻击缺失仍在。下文“尚未编译”等描述为初次接手时的历史快照。
...
run55 的时间线使用事件 rel_ms（steady_clock）；结果 duration_ms 由 Observer 累加 diff。完整事件流在 299998ms 结束，与 duration_ms=300008 基本一致。最后 boss_hp 事件约 150504ms 仅表示 boss 采样结束，不能据此推断时钟两倍差异。后半段角色位置仍在采样，结束前全队在地图 571，已离开 NAXX（533）；需查死亡后复活/离场与终态判定的交互，不能仅凭每名角色死过一次断言同时全灭。
...
另列两项独立问题：主坦未形成持续攻击；全员有死亡记录、随后离开副本，但最终为 timeout。两项都不能由“DPS 引擎随机性”代替调查。

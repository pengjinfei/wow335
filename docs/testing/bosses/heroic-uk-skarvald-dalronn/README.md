...
更新：2026-09-06；当前状态：双 boss 正常规则零死亡/近零死亡击杀（KillGateSpawn 框架改动后）；房间 10 只小怪清理未纳入（隔离 boss 战验证）。
...
连续 3 次击杀。双 boss 均被击杀：达尔隆先死→召唤幽灵→斯卡瓦德后死→击杀判定（KillGateSpawn 生效）。run96/a2 有 1 死（bot 阵亡但队伍仍完成击杀）。

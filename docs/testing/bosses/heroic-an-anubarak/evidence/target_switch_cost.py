...
作废原因：群体技能会在同一刻命中多个目标，被本脚本当成「换目标」——实测换目标的空档中位
只有 0.1-0.2 秒，比同目标的出手间隔（法师 1.0s / 萨满 1.2s）还短，算出来的「切换损失」是假的。
真正有效的口径见第二十四轮：按 tick 统计主输出技能「被推入 vs 轮得到」的比例。

原说明：目标切换的摩擦成本：每次换目标要停多久，折算成损失了多少 DPS。

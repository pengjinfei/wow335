...
> ⚠️ **两个用法陷阱（勘测阿努巴拉克时都踩了）**：
> 1. `vmap_floor_*` 是从 z+2 **向下搜 50 码**找地面，传偏高的 z 只是搜索起点；
>    但 **`los` 是从 `(x1,y1,z1+2)` 到 `(x2,y2,z2+2)` 量的**。把"向下搜的起点"当 z1 传
>    （z1=240 而地面 223）等于在离地 18 码处量视线，墙沿坑沿全越过去 —— 得到的 `los=true` 是假的。
>    **正确做法两步**：先拿 `vmap_floor_from`，再用该地面高度当 z1 复量，只信第二遍。
> 2. 它是**静态 vmap 探针，看不到战斗中才关上的门**。boss 带 `DOOR_TYPE_ROOM` 门时
>    （阿努巴拉克有三扇），准备点与开怪点必须都在 boss 那一侧，否则远程会被关在门外、
>    治疗因 `IsWithinLOSInMap` 选不到坦克而全场空转。
...
- **准备点 (527, 248, 224.35)**：距 boss 24.0 码 ✓，**以 224.35 为 z1 实测视线通**，在门线以南。
  距区间圆心 24.3 码 < 32。（第一版 (551,280) / 第二版 (551,224) 都是错的，原因见上面两个陷阱。）
- **开怪点 (543, 248, 223.44)**：16.0 码，地面高度实测视线通，同在门线以南。
- 内圈地面 y 240–251 约为 x 527–575，向南收窄；x ≤ 523 无地面。
  **门内侧距 boss 最远就是 24 码**（东西两翼 y=248/251）。

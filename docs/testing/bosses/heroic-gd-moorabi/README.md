...
- seq2：**只清掉 2/5** 就被团灭（其余两场是清完后在恢复期减员）。

三场的共同点是 **29819 Lancer**：seq1/seq5 的盗贼之死，死前 3 秒承伤 12,677 / 18,519 全部来自它。
（⚠ 这里**曾**写过「治疗有 26 秒 heal 空档」——那是漏看中间采样的错值，实际 17.7 秒且发生在
快团灭之后，是结果不是原因；已在本页下方与 LESSONS 第 21 条纠正。）

## 2026-09-18 晚：前置清怪头寸量化（run664–667 的 20 场，只读，未改代码）

口径：窗口 = `[0, 前置怪最后一只死亡时刻]`，即清怪段本身（不包含恢复期）。
⚠ 不能用「boss 首次伤害时刻」当窗口边界——run664 seq2 的 29305 在 194ms 就有伤害事件（上一场残留），
会把整个窗口压成 0.2 秒。

### 结论：靶子不是「治疗停了」，也不是「伤害太高」

| 指标 | kill 组（n=16） | fail 组（n=4） | 判定 |
|---|---|---|---|
| 前置怪 DTPS | 1,529 | 1,776 | 不可区分（fail 组只高 16%，n=4）|
| heal 采样密度（次/10s） | 5.8 | **6.5** | **fail 组治疗反而更多** |
| 最大 heal 空档 | 7.7s | 5.6s | **fail 组反而更短** |
| 全队输出速率（partyDPS） | 5,921 | 5,919 | **完全相同** |

四项靶子指标全部**不可区分或方向相反**。特别是 partyDPS 5,921 vs 5,919——
**输出不是瓶颈**，「小怪撒着打不死」在莫拉比这组上也不成立。

### 靶子不是「Lancer 没被坦克拉住」，是 **Retaliation 反伤（22858）**

> **2026-09-18 晚第二轮修正。** 上一轮把「Lancer 打盗贼 287,689 > 打坦克 185,254」
> 当成「Lancer 没被拉住」的证据。**那个口径是错的**——它把 Lancer 的
> **Retaliation 反伤**算进了 Lancer 的「目标选择」。详见下面「原口径错在哪」。

#### 原口径错在哪（可读代码证明，不依赖实测）

29819 的 SmartAI：每 12–20 秒放 `40546 Retaliation`（**给自己上 aura**，`DurationIndex 28` = 5000ms）；
40546 的 Effect1 = `APPLY_AURA` + `SPELL_AURA_PROC_TRIGGER_SPELL(42)`，`TriggerSpell = 22858`，
`ProcTypeMask 0x28` = `DONE_MELEE_AUTO_ATTACK|TAKEN_MELEE_AUTO_ATTACK|TAKEN_SPELL_MELEE_DMG_CLASS`，
`ProcChance 100`。22858 是瞬发、近战距离、Effect = `WEAPON_DAMAGE(58)`。

⇒ **谁在 aura 窗口内碰了 Lancer，22858 就打谁一次。** 它的目标 = **攻击者**，与 Lancer 的仇恨无关。
22858 的 `Attributes = 0x00040000` 只有 `SPELL_ATTR0_DO_NOT_SHEATH`，
**没有任何目标选择/无视仇恨相关的位**；`spell_script_names`、`spell_target_position`、`spell_threat` 均无条目。

**验证（1:1 事件配对，run664–667）**：154 次**落地**（`miss=0`）的 22858 施法，
**全部**能在同一 `(rel_ms, target)` 上找到 1 条 damage 事件；反查「该 target 在 300ms 内是否打过 Lancer」
——**131/131 全部命中，0 例外**。配对口径可信。

#### 修正后的落点分解（run664–667，20 场）

| 类别 | 坦克 | 非坦克 | 说明 |
|---|---|---|---|
| **Lancer 普攻（真的选谁）** | 87,535 | **137,470** | 坦克占 38%；逐场「坦克 > 全部非坦克」只有 **5/19 场** |
| **Retaliation 反伤 22858** | 69,193 | **286,855** | 坦克占 19%；落点 = **谁打了 Lancer**，不是 Lancer 选的 |
| Impale 流血 58978（真的选谁） | 44,177 | 34,407 | 英雄难度版（`spelldifficulty_dbc` 55622→58978）|

**原口径那 287,689（盗贼）里，230,489（80%）是反伤**，真正的普攻只有 57,200。
坦克那 185,254 里 69,193 也是反伤，真正普攻 87,535——**坦克的普攻承伤本来就比盗贼多**。
「Lancer 打盗贼更多」是**盗贼打 Lancer 更多**（近战 541 次命中，坦克 480 次）被反伤打回来的结果。
...
| run666/s1 @39959 | rogue=12,677(4h) > mage=5,330 > heal=4,782 > tank=1,967 |
| run666/s5 @41526 | rogue=18,519(4h) > self=4,296 > tank=0 |
| run667/s2 @42132 | **tank=12,367(7h)** > self=4,564（这场是坦克在打）|

**统计上反伤落点 ≈ 我方对 Lancer 的输出分配**：rogue 541 次命中 / tank 480 / shaman 187 / mage 160 / heal 27。
所以盗贼死得多，是因为**盗贼打 Lancer 打得最多**，而 aura 期间没人收手。

#### 死因分解（窗口 `[t-12s, t+2.5s]`，n=39 场前置玩家死亡）
...
run666 seq1/seq5 盗贼之死（12,677 / 18,519）**确实 100% 来自 Lancer，但全部是反伤**，
不是「Lancer 在追着盗贼打」。

#### 顺带纠正：坦克其实**很少嘲讽 Lancer**

19 场里 **9 场坦克从未对 Lancer 放过制裁之手(62124)**；首次嘲讽中位在开怪后 ~3.0 秒。
嘲讽前后普攻落点：嘲讽前 `tank 35 / shaman 12 / rogue 12 / mage 6 / heal 2`，
嘲讽后 `tank 25 / shaman 7 / heal 1 / rogue 1`。**嘲讽是有效的**（之后坦克占多数），
问题是没有稳定使用——但**它不是本轮的瓶颈**（反伤不吃嘲讽）。

### 输出分配是健康的（不是「撒着打」）

前置窗口内全队输出 6,840,013，按目标分布：EarthshakerA 29.7%、EarthshakerB 29.6%、
FireWeaver 18.9%、Lancer 16.7%、Inciter 4.9%。四个 dps 职业对每只怪的分配均衡（无单一职业偏斜）。
...
- 我一度说「治疗有 26 秒完全空档」：**错**。我漏看了中间的 `21784` 采样，实际最大空档是
  **17.7 秒**，且它发生在快团灭之后（25.4s 之后只剩残局），不是原因而是结果。
- 我一度用 `actor_entry=797` 统计玩家行为：**错**，`actor_entry` 是怪物 entry，玩家要用 `source_guid`。

### 控制链降为第二变量（前置五只 `type=7`，确实可用）
...
⚠ 但根据上面刚量出的头寸，**它不再是首选**：瓶颈是单只 Lancer 的目标选择，
不是接敌数量、也不是输出。先查 Lancer。

⚠ LESSONS 第六节待办第 0 条（施法型取值补「走过去」链条）管的是**驱散/复活/团队 buff**——
隔墙放不出去。run666 seq2 不是那条：那里全队对怪 `los=true`，属于另一条链路（但那条的
「26 秒空档」本身已被上面纠正为 17.7 秒且发生在团灭之后）。
...
1. **「先查 29819 Lancer 为什么没被坦克拉住」——已答完，答案是不存在这个问题**（零编译成本）。
   ⚠ 下面这条 2026-09-18 上一轮写的待办**已被当晚第二轮修正推翻**，保留仅作历史记录：
   当时以为「它打盗贼 39.3%、打坦克 25.3%，19 场里 13 场打盗贼更多」。
   实际 22858 是 `40546` aura 的 proc 反伤（谁打我打谁），不是目标选择技能；
   盗贼那 287,689 里 80% 是反伤。结论见上面「靶子不是…」一节。
...
4. **boss 阶段 `CanNotReachTarget`（run640 seq4）仍未归因**，本轮未复现，与前两项是不同的事。

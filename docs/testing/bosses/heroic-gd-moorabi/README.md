...
### 真正的靶子：29819 Lancer 没被坦克拉住

前置窗口内，单只 Lancer 的伤害去向：

| 目标 | 命中 | 伤害 | 占比 |
|---|---|---|---|
| **盗贼** | 79 | **287,689** | **39.3%** |
| 坦克 | 143 | 185,254 | 25.3% |
| （它自己/宠物） | 22 | 94,141 | 12.9% |
| 萨满 | 44 | 79,237 | 10.8% |
| 法师 | 17 | 54,229 | 7.4% |
| 治疗 | 8 | 26,142 | 3.6% |

**它打盗贼比打坦克还多**。而 run666 seq1/seq5 的死亡就是它造成的：

| 场次 | 死者 | 死前 3 秒承伤 | 来源 |
|---|---|---|---|
| seq1 | 盗贼 | 12,677 | 全是 29819（4883+3505+4289）|
| seq5 | 盗贼 | **18,519** | 全是 29819（5054+3769+4729+4967）|
...
1. **先查 29819 Lancer 为什么没被坦克拉住**（零编译成本）：看它的技能（22858，前置窗口内 248 次施法、
   对盗贼 113 次、对坦克 95 次）是不是一个**无视仇恨的目标选择技能**，还是仇恨移交/嘲讽链的问题。
2. 若是无视仇恨的随机目标技能 → 属于 boss 机制，要看它能不能被规避/控制，而不是「坦克没拉住」。
   若是仇恨问题 → 属共享层坦克仇恨链，需单独量。
3. **接控制链仍是有价值的第二变量**（分批拉能减少同时接敌数量），但**不再是首选**——
   本轮数据显示瓶颈是单只 Lancer 的目标选择，不是接敌数量或输出。

...
Dark Matter 的单变量已在隔离 r32-equivalent tree 实施；受控替换 startup gate 已通过（world ready、场景已加载、FIFO IDLE），并启动排除 lifecycle 的 run761/attempt `1788427984` execution smoke；cycle 11 的 FIFO 状态为仍在运行、前置已于 173.449 秒完成且正在 recovery（175.582 秒），尚未进入可观测动态段，因此 INFO=0 不是执行失败：Marnak 每 30 秒生成 28237，5 秒后向随机玩家当时坐标 `MovePoint`，再在按初始距离确定的 3–4.5 秒后自爆。r32 五场均有其伤害（kill 12,514；wipe 12,682/14,158/9,830/5,083），且同一爆炸可记录多个受击目标（例如 run746 的 346.071 秒对819=4,801、820=4,858）；HoS 没有 Dark Matter node。隔离 action 仅在 combat、5 码内发现 28237 时以 `ACTION_EMERGENCY+10` 调既有 `MoveAway` 至12码，不改目标/仇恨/raidtest；实际 action 才写 `tribunal-dark-matter` INFO（bot、触发距离、MoveAway 返回）。隔离 `worldserver -j4` 已成功构建。执行 smoke 的预设门槛是至少一次 INFO action 且可对照随后的非零 28237 命中；cycle 12 已在动态 observing 段直接验到 9 条 INFO（6 次 `moved=true`、3 次 `false`，首次两名 bot 于 action 时距 4.58/3.11 码），因此 trigger 与 bot 自主移动执行已通过。cycle 13 终态 flush 后计数为 11 条（7 true/4 false）；run761/attempt `1788427984` 于 497.206 秒以 5 deaths wipe 结束，`raidtest_events` 有一条 actor 28237 对 roster DPS 829 的非零伤害 5,197（rel 362.122 秒）。故 trigger、移动及非零命中三项 execution gate 均通过；这仅证明节点在正常规则下执行且伤害仍可观测，绝不将此 smoke/wipe 计入 lifecycle 或推断效果。cycle 13 已在终态后恢复 r32 binary；受控 restart startup/FIFO gate 为 ready/IDLE。该 smoke 永不进入 lifecycle。不得与 r34 同二进制混跑。

...
| heroic-uk-ingvar，五人 heroic5-disc-v1 | 隔离 Boss：run116/a1 112.716s（2 死）、run130/a1 115.014s（3 死）击杀，均覆盖 P1→复活→P2；下层与骑手平台局部扫描均无战斗终止 | 隔离 Boss 可击杀；前置 3 骑手、旧实例 worldport、全灭后夹具恢复均有单项证据。完整客户端重提取的 map 574 已生成 `5743132.mmtile`，但当前 MMap 无下层至平台的严格路线；实验代码未保留 | 服务端双向连通分量与正常场景过渡机制分析；确认可复现修复后再提交代码并做含前置怪冷启动回归 |

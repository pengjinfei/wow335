...
| heroic-uk-ingvar，五人 heroic5-disc-v1 | 隔离 Boss：run116/a1 112.716s（2 死）、run130/a1 115.014s（3 死）击杀，均覆盖 P1→复活→P2；路线勘测 run145/a1 在 0ms 被 MMap 完整性核验拒绝 | 隔离 Boss 可击杀；前置 3 骑手、旧实例 worldport、全灭后夹具恢复均有单项证据。map 574 重建后房间起点是有效 MMap 多边形，但上层平台零位移仍为 `PATHFIND_NOT_USING_PATH` fallback，故真实路线和含前置怪冷启动未验收 | 修复/确认该段 MMap 覆盖后再写楼梯点，完成至少一次含前置怪的冷启动回归，再评估伤亡与稳定率 |

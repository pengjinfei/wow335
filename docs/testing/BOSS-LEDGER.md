...
| heroic-uk-ingvar，五人 heroic5-disc-v1 | 隔离 Boss：run116/a1 112.716s（2 死）、run130/a1 115.014s（3 死）击杀，均覆盖 P1→复活→P2；完整链路：run142 已验证分段导航状态机 | 隔离 Boss 可击杀；前置 3 骑手、旧实例 worldport、全灭后夹具恢复均有单项证据。真实上层路线及含前置怪冷启动尚未验收 | 勘测并配置楼梯路径点，完成至少一次含前置怪的冷启动回归，再评估伤亡与稳定率 |

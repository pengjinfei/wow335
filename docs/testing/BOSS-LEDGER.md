...
| heroic-uk-ingvar，五人 heroic5-disc-v1 | 隔离 Boss：run116/a1 112.716s（2 死）、run130/a1 115.014s（3 死）击杀，均覆盖 P1→复活→P2；路线探针 run149/150/152 均无战斗终止，run151 验证候选数据的下层点可走 | 隔离 Boss 可击杀；前置 3 骑手、旧实例 worldport、全灭后夹具恢复均有单项证据。已用完整 3.3.5 客户端重提取 map 574，并修正平台 tile 为 `5743132`；默认网格发生顶点溢出，1600 网格可生成目标 tile，但 Boss/骑手给定坐标及骑手高程候选未投影到可走多边形 | 实现或调用候选 DataDir 的局部 XY/Z 投影扫描，取得连续可走点后勘测楼梯分段，完成至少一次含前置怪的冷启动回归 |

...
## 2.5 验证提速（2026-09-15 起）

一轮 5 场的墙钟几乎全是战斗时间（run 500：墙钟 22 分钟 vs 战斗 21.6 分钟），场间准备开销可忽略，
所以提速只能从下面几处找，且不得改变被测机制：

- **打输就收尾**（已做，mod-raidtest `22107fd`）：有人阵亡 + boss 脱战回满血 + 残存者脱战 → 立即判 wipe，
  不再空跑到 480 秒超时。本会话 5 次 timeout 每次白跑约 200 秒。
- **序贯早停**（约定）：一轮里前 3 场 0 击杀、且改动针对的中间量毫无位移 → 中止该轮，记为「提前中止」。
  **必须事先声明规则**，不能看完数据再决定停不停。本会话 run 507/511/512/515 都是这么处理的。
- **并行多实例**（未做，最大杠杆）：`MapUpdate.Threads` 目前为 1，bot 的 AI tick 实测 0.69 秒/个。
  并发前必须先验证「并发后 tick 间隔和击杀时间分布不变」，否则等于换了个更差的被测对象，结论失真。
- **筛选型场景**（未做）：如「boss 从 25% 起」只测末段改动，一场 60–80 秒。**只能当筛选**，
  留下的候选仍要用完整场次复核；结论里必须标注是筛选场景。

**不可用**：加快服务器时间流速、削 boss 血量/伤害、给 bot 加属性——这些改变被测机制本身。

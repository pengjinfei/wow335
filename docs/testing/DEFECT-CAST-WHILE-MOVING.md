...
> 状态：**修复已实现、已编译、已在阿努巴拉克两轮 10 场验证（run 457 / 459）——行为正确，但本文对瓶颈的归因被证伪。**
> 设计与实现见 [DEFECT-CAST-WHILE-MOVING-PLAN.md](DEFECT-CAST-WHILE-MOVING-PLAN.md)，验证数据见
> [阿努巴拉克记录第四轮](bosses/heroic-an-anubarak/README.md)。
> 实测：读条治疗被拒 258 次里"移动中"只有 19 次（~7%），且几乎全是穿刺击飞/躲刺；主因是 GCD 争用与苦修引导。
> 本文下面的 FAILED 数字当时被全部算成"移动"，这是错的——run 460 补上 prepare 结果码日志后，牧师 365 次 FAILED
> **全部是 `SPELL_FAILED_NO_POWER`**：牧师在每场 150–210 秒耗尽法力。"不是蓝量"那条排除项也是误读
> （把回蓝技能的 IMPOSSIBLE 当成"没触发"，实际是冷却中/包里没药水）。真正的瓶颈与后续方向见
> [阿努巴拉克记录 run 460 小节](bosses/heroic-an-anubarak/README.md)。
> ⚠️ 计划文档 §1 纠正了本文三处偏差：`:3735` 那段实际是死代码（`GetCastTime()` 在 `prepare()` 前恒为 0），
> 真正拒绝施法的是 core `Spell::prepare` 的 `SPELL_FAILED_MOVING`；引导法术（苦修等）同样被拒；
> `:3948` 在 `prepare()` 后 `delete spell` 有双重释放隐患。本文其余证据与结论仍然成立。

...
1. **清完后被 boss 拖入**（run407 a1 萨满死于泰蕾斯特拉、a4 恢复期全队 combat=1 直到超时）：最后一只是被羊的治疗，
...
3. **剑刃乱舞每场开怪 15 秒左右都放出来了**：乘子先按 `IsPositive()` 放行了正面自身增益，名单没起作用。
   代码已改（先查名单再看正负），**但这一处改动还没编译**，当前运行的二进制不含它。

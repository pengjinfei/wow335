...
> 📖 **新会话/同事接手前请先读 [CLAUDE.md](CLAUDE.md)**——它是一份完整的接手续航文档（项目定位/环境/进度/技术发现/下一步）。
...
- [x] **阶段 1：环境构建 + 文档（已完成）**
- [x] **阶段 2：mod-raidtest 模块开发（A 阶段：管线/事件流/命令层全通；B1：AI激活/顶装/实例重置/可信判定，首个真实 wipe 收官）**
- [ ] 阶段 3：NAXX 攻坚（B2 头号项=无 master bot 施法循环 → 打通首个 boss 击杀 → 优化闭环）
- [ ] 阶段 4：按副本逐个攻坚
